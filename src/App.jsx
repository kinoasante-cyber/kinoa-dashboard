...
function SkeletonRow() {
  return (
    <tr className="skeleton-row">
      {[140, 90, 150, 50, 180, 110, 70, 110, 70, 80].map((w, i) => (
        <td key={i}>
...
export default function App() {
  ...
  const rdvCeSemaine = useMemo(
    () => patients.filter(p => isThisWeek(p.prochaineVisite)).length,
    [patients],
  );
  ...
  function SortTh({ label, k }) {
    ...
    return (
      <th onClick={() => handleSort(k)} className={`th-sortable${active ? ' th-active' : ''}`}>
        {label} <span className="sort-arrow">{active ? (sortDir === 1 ? '↑' : '↓') : '↕'}</span>
      </th>
    );
  }
      ...
      <aside className="sidebar">
        <div className="sidebar-logo">
          ...
          <div>
            ...
            <p className="logo-clinic">OrthèseGo</p>
          </div>
        </div>
        ...
        <nav className="sidebar-nav">
          ...
          <a href="#" className="nav-item">
            <IconAlert /> Alertes
            {counts.ROUGE > 0 && <span className="nav-badge">{counts.ROUGE}</span>}
          </a>
        </nav>
        ...
        <div className="sidebar-footer">
          ...
          <div>
            <p className="sidebar-user">Julien Côté</p>
            <p className="sidebar-role">Orthotiste</p>
          </div>
        </div>
      </aside>
      ...
      <main className="main">
        <header className="page-header">
          <div>
            ...
            <p className="page-sub">
              Clinique OrthèseGo · {today.toLocaleDateString('fr-CA', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
            </p>
          </div>
          {!loading && (
            <button className="refresh-btn" onClick={refetch} title="Actualiser les données">
              <IconRefresh />
            </button>
          )}
        </header>
        ...
        <div className="stats-grid">
          {loading ? (
            ...
            <>
              <StatCard label="Patients actifs"   value={patients.length}     sub="clinique OrthèseGo"           icon={<IconUsers />}    accent="#E8470A" />
              ...
              <StatCard label="Adhérence moyenne"  value={`${avgAdherence}%`} sub="tous patients confondus"      icon={<IconChart />}    accent="#22C55E" />
            </>
          )}
        </div>

        {/* Filters — masqués pendant chargement/erreur */}
        {!loading && !error && (
          <div className="toolbar">
            <div className="search-box">
              ...
              <input
                ...
                placeholder="Rechercher un patient, diagnostic, orthotiste…"
                value={search}
            ...
            <div className="filter-pills">
              {[FILTER_ALL, 'ROUGE', 'JAUNE', 'VERT'].map(s => (
                <button
                  ...
                  className={`pill${filterStatut === s ? ' pill--active' : ''}`}
                  style={filterStatut === s && s !== FILTER_ALL
                    ? { background: STATUTS[s].bg, color: STATUTS[s].color, borderColor: STATUTS[s].dot }
                    : {}}
                >
                  {s === FILTER_ALL ? 'Tous' : STATUTS[s].label}
                  <span className="pill-count">
                    {s === FILTER_ALL ? patients.length : counts[s]}
                  </span>
                </button>
              ))}
            </div>
        ...
        <div className="table-wrap">
          {error ? (
            ...
            <table className="patient-table">
              <thead>
                <tr>
                  <SortTh label="Patient"        k="nom" />
                  <th>Téléphone</th>
                  <th>Email</th>
                  <SortTh label="Âge"            k="ddn" />
                  <th>Diagnostic</th>
                  <th>Orthotiste</th>
                  <SortTh label="Statut"         k="statut" />
                  <SortTh label="Adhérence"      k="adherence" />
                  <SortTh label="Dernière visite" k="derniereVisite" />
                  <SortTh label="Prochain RDV"   k="prochaineVisite" />
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="10" className="empty-state">Aucun patient trouvé.</td>
                  </tr>
                ) : (
                  filtered.map(p => (
                    <tr
